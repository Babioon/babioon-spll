...
class JFormFieldRules extends JFormField
{
	...
	protected function getInput()
	{
		...
		// name must be a uniq within the config.xml file 
		$name = $this->element['name'] ? (string) $this->element['name'] : 'rules';
		...
		else
		{
			...
			if (is_null($assetId))
			{
				// there are two reasons why we don't have an asset id at this moment
				// one is that we don't have a saved configuration for that asset
				// two is that this is a component.section rules item
				// we check if it is the second reason
				if (trim($component) != '' && trim($section) != '')
				{
					// try to find the asset id by name == component.section.
					$db = JFactory::getDbo();
					$query = $db->getQuery(true);
					$query->select($db->quoteName('id'));
					$query->from($db->quoteName('#__assets'));
					$query->where($db->quoteName('name') . ' = ' . $db->quote(trim($component).'.'.trim($section)));
					$db->setQuery($query);
					$assetId = (int) $db->loadResult();
		...
		// Prepare output
		$html = array();
		$html[] = '<div id="'.$name.'-permissions-sliders" class="pane-sliders">';
		$html[] = '<p class="rule-desc">' . JText::_('JLIB_RULES_SETTINGS_DESC') . '</p>';
		$html[] = '<ul id="'.$name.'" class="rules">';
		...
		foreach ($groups as $group)
		{
			...
			$html[] = '<th class="actions" id="'.$name.'-actions-th' . $group->value . '">';
			$html[] = '<span class="acl-action">' . JText::_('JLIB_RULES_ACTION') . '</span>';
			$html[] = '</th>';

			$html[] = '<th class="settings" id="'.$name.'-settings-th' . $group->value . '">';
			...
			{
				$html[] = '<th id="'.$name.'-aclactionth' . $group->value . '">';
			...
			{
				$html[] = '<tr>';
				$html[] = '<td headers="'.$name.'-actions-th' . $group->value . '">';
				...
				$html[] = '<td headers="'.$name.'-settings-th' . $group->value . '">';
				...
				if ($canCalculateSettings)
				{
					$html[] = '<td headers="'.$name.'-aclactionth' . $group->value . '">';
		...
		$js = "window.addEvent('domready', function(){ new Fx.Accordion($$('div#$name-permissions-sliders.pane-sliders .panel h3.pane-toggler'),"
			. "$$('div#$name-permissions-sliders.pane-sliders .panel div.pane-slider'), {onActive: function(toggler, i) {toggler.addClass('pane-toggler-down');"
			. "toggler.removeClass('pane-toggler');i.addClass('pane-down');i.removeClass('pane-hide');Cookie.write('jpanesliders_".$name."_permissions-sliders"
			. $component
			. "',$$('div#$name-permissions-sliders.pane-sliders .panel h3').indexOf(toggler));},"
			...
			. JRequest::getInt('jpanesliders_'.$name.'_permissions-sliders' . $component, 0, 'cookie') . ", show: "
			. JRequest::getInt('jpanesliders_'.$name.'_permissions-sliders' . $component, 0, 'cookie') . ", alwaysHide:true, opacity: false}); });";
	...
	{
		return $this->element['section'] ? (string) $this->element['section'] : '';
	}
